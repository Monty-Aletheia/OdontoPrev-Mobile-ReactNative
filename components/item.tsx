...
const ListItems: React.FC<ListItemProps> = ({ consultation }) => {
  ...
  return (
    <View className="flex-row justify-between bg-white rounded-md shadow-lg m-5 pt-3 pb-3">
      <View>
        <Image
          source={require("../assets/images/profile_list_icon.png")}
          className="w-20 h-20"
        />
      </View>

      <View className="ml-2 flex-1 justify-center">
        <Text className="text-lg font-bold color-dark_blue">
          {formatName(consultation.patient.name)}
        </Text>
        <Text className="text-lg color-dark_blue">
